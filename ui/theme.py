...
/* ── Trendwende-Ampel-Karte (.tampel) ─────────────────────────────
   Mobile: alles untereinander gestapelt (Boxen klar abgegrenzt).
   Desktop: 2-spaltiges Grid (Lichter | Text/Action), darunter 4er-Stats
   und 2er-Status. Lichter behalten ihre Inline-Maße. */
.tampel {
  background: var(--panel);
  border: 1px solid var(--border);
  border-radius: var(--radius-lg);
...
.tampel__main { display: flex; flex-direction: column; gap: 12px; }
.tampel__lights {
  background: var(--panel-2);
  border: 1px solid var(--border);
  border-radius: var(--radius-md);
...
.tampel__body { display: flex; flex-direction: column; gap: 8px; min-width: 0; }
.tampel__verdict { font-size: 1.06rem; font-weight: 800; letter-spacing: .02em; line-height: 1.2; }
.tampel__reason { font-size: .85rem; color: var(--text); line-height: 1.5; }
.tampel__action {
...
.tampel__stat-label {
  font-size: .58rem; letter-spacing: .1em; font-weight: 800;
  color: var(--muted); text-transform: uppercase;
}
.tampel__stat-value {
  font-size: .92rem; font-weight: 700; color: var(--text);
  margin-top: 4px; overflow-wrap: anywhere; line-height: 1.25;
}
.tampel__states {
...
.tampel__state-icon { font-size: 1.3rem; line-height: 1; flex: 0 0 auto; }
.tampel__state-body { min-width: 0; }
.tampel__state-title { font-size: .8rem; font-weight: 800; line-height: 1.2; }
.tampel__state-detail {
...
@media (min-width: 981px) {
  /* Schmalere Seitenbreite — verhindert „breit gezogen" */
  .main .block-container { max-width: 1140px; }

  /* Hero / KPI-Karten kompakter */
  .summary-hero { padding: 18px 22px; }
  .change-card { padding: 13px 15px; }
  .change-card .change-value { font-size: 1.05rem; }

  /* MA-Distanz: 4 Spalten statt 2, damit alles in eine Zeile passt */
  .mobile-ma-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; }

  /* Trendwende-Ampel als 2-Spalten-Grid: Lichter links, Text rechts.
     Stats und Status spannen darunter über die ganze Breite. */
  .tampel {
    padding: 18px 22px;
    display: grid;
  ...
  .tampel__head   { grid-area: head;   margin-bottom: 0; }
  .tampel__main   { display: contents; }
  .tampel__lights { grid-area: lights; align-self: stretch; }
  .tampel__body   { grid-area: body; }
  .tampel__stats  { grid-area: stats; grid-template-columns: repeat(4, minmax(0, 1fr)); margin-top: 0; }
  .tampel__states {
    grid-area: states;
